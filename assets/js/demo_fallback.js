// Demo Page JavaScript - Fallback Version with Enhanced Error Handling
class DemoController {
    ...
    initializeSounds() {
        this.sounds = {
            click: this.createOscillatorSound(800, 0.05, 0.1),
    ...
    // Fallback Maps 초기화 - 네이버 지도 API 없이도 작동하는 대체 솔루션
    initializeFallbackMaps() {
        this.baseHeatmapData = [
            // 의정부시 (고위험 지역)
            {lat: 37.741, lng: 127.047, risk: 0.9, area: '의정부역 주변'},
            {lat: 37.742, lng: 127.048, risk: 0.85, area: '의정부 민락동'}, 
            {lat: 37.740, lng: 127.046, risk: 0.8, area: '의정부 신곡동'},
            
            // 고양시
            {lat: 37.658, lng: 126.834, risk: 0.8, area: '일산 라페스타'},
            {lat: 37.659, lng: 126.835, risk: 0.75, area: '일산서구 주엽동'}, 
            {lat: 37.657, lng: 126.833, risk: 0.7, area: '일산동구 장항동'},
            
            // 파주시
            {lat: 37.767, lng: 126.780, risk: 0.7, area: '파주 금촌 로데오거리'},
            {lat: 37.768, lng: 126.781, risk: 0.65, area: '파주 금촌역 인근'}, 
            
            // 기타 경기북부 지역들
            {lat: 37.593, lng: 127.130, risk: 0.6, area: '구리 돌다리역'},
            {lat: 37.636, lng: 127.224, risk: 0.65, area: '남양주 다산신도시'},
            {lat: 37.826, lng: 127.045, risk: 0.5, area: '양주 덕계역'},
            {lat: 37.903, lng: 127.060, risk: 0.55, area: '동두천 중앙역'},
            {lat: 38.146, lng: 127.200, risk: 0.4, area: '포천시청 일대'}
        ];
        
        this.currentHeatmapData = this.generateHeatmapPoints(this.heatmapIntensity);
        this.addFallbackPatrolData();
        this.updateHighRiskAreasUI();
        this.updatePatrolUnitsUI(false);
        this.mapInitialized = true;
        
        console.log('Fallback maps initialized successfully');
    }

    addFallbackPatrolData() {
        const patrolPoints = [
    ...
    updateHeatmapAppearance() {
        ...
        // Visual feedback for heatmap update
        const heatmapContainer = document.querySelector('#riskHeatmap');
        if (heatmapContainer) {
            heatmapContainer.classList.add('heatmap-updating');
            setTimeout(() => heatmapContainer.classList.remove('heatmap-updating'), 500);
        }
    }
        ...
        this.heatmapAutoUpdateTimer = setInterval(() => {
            ...
            const autoUpdatedData = this.currentHeatmapData.map(p => {
                ...
                return {
                    lat: p.lat, 
                    lng: p.lng, 
                    risk: Math.max(0, Math.min(1, newIntensity)),
                    area: p.area
                };
            });
    ...
    stopHeatmapAutoUpdate() {
        ...
        const statusEl = document.getElementById('heatmapAutoUpdateStatus');
        if(statusEl) {
            statusEl.textContent = '(자동 업데이트 중지됨)';
        ...
        try {
            ...
            this.predictionChart = new Chart(ctx, {
                ...
                data: {
                    ...
                    datasets: [{
                        label: '예측 위험도 (%)',
                        data: hours.map(() => Math.random() * 60 + 20),
                        borderColor: '#3b82f6',
            ...
            console.error('Prediction chart init failed:', e);
        }
    }
    ...
    initPerformanceChart() {
        ...
        try {
                ...
                data: {
                    ...
                    datasets: [
                        ...
                        },
                        {
                            label: '순찰 효율성 (%)',
                            type: 'bar',
                            data: Array.from({length: 12}, () => Math.random() * 20 + 75),
            ...
            console.error('Performance chart init failed:', e);
        }
    }
    ...
    startPatrolSimulation() { 
        ...
        this.simulationTimer = setInterval(() => { 
            ...
            if (Math.random() < 0.2) this.generateRandomEvent(); 
        }, 2000); 
    }
    ...
    updatePatrolUnitsUI(isRunning) { 
        ...
        if (!isRunning && !this.simulationRunning) { 
        ...
        this.patrolUnitsData.slice(0, 5).forEach(unit => { 
            if (isRunning) { 
                ...
                if (rand < 0.6) { 
                    ...
                    unit.status = '순찰중'; 
                    unit.icon = 'fa-person-walking-arrow-right'; 
                } else { 
                    unit.status = '출동'; 
                    unit.icon = 'fa-truck-medical'; 
                } 
            } 
    ...
    updateHighRiskAreasUI() { 
        ...
        topRisks.forEach((riskPoint, index) => { 
            ...
            if (riskPoint.risk >= 0.8) { 
                bgColor = 'bg-red-100'; 
                textColor = 'text-red-800'; 
            } else if (riskPoint.risk >= 0.5) { 
                bgColor = 'bg-orange-100'; 
                textColor = 'text-orange-800'; 
            } 
            const areaName = riskPoint.area || `지역 ${index + 1}`;
            container.insertAdjacentHTML('beforeend', `<div class="bg-white rounded-lg p-3 border hover:shadow-sm transition-shadow"><div class="flex justify-between items-center mb-1"><span class="text-sm font-medium text-gray-800">${areaName} (예시)</span><span class="text-xs ${bgColor} ${textColor} px-2 py-1 rounded-full">${riskPercent}%</span></div><p class="text-xs text-gray-600">${details[index % details.length]}</p></div>`); 
        }); 
    }
    ...
    addLiveReport(category, details) { 
        ...
        const statusColors = { 
            ...
            "대응완료": "bg-green-100 text-green-800" 
        }; 
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    console.log('Demo application initialized with fallback maps');
});