// Demo Page JavaScript - Naver Maps Version with Complete Implementation
class DemoController {
    ...
    // 네이버맵 초기화 함수 - Leaflet에서 네이버맵으로 완전 변경
    initializeMaps() {
        // 순찰 지도 초기화
        ...
        if (document.getElementById('riskHeatmap')) {
            try {
                ...
                this.baseHeatmapData = [
                    // 의정부시 (고위험 지역)
                    {lat: 37.741, lng: 127.047, risk: 0.9}, {lat: 37.742, lng: 127.048, risk: 0.85}, 
                    {lat: 37.740, lng: 127.046, risk: 0.8}, {lat: 37.743, lng: 127.049, risk: 0.75},
                    {lat: 37.738, lng: 127.045, risk: 0.7}, {lat: 37.745, lng: 127.050, risk: 0.65},
                    
                    // 고양시 (일산서구, 일산동구, 덕양구)
                    {lat: 37.658, lng: 126.834, risk: 0.8}, {lat: 37.659, lng: 126.835, risk: 0.75}, 
                    {lat: 37.657, lng: 126.833, risk: 0.7}, {lat: 37.660, lng: 126.836, risk: 0.65},
                    {lat: 37.648, lng: 126.844, risk: 0.6}, {lat: 37.668, lng: 126.824, risk: 0.55}, 
                    {lat: 37.650, lng: 126.850, risk: 0.5},
                    
                    // 파주시
                    {lat: 37.767, lng: 126.780, risk: 0.7}, {lat: 37.768, lng: 126.781, risk: 0.65}, 
                    {lat: 37.766, lng: 126.779, risk: 0.6}, {lat: 37.770, lng: 126.785, risk: 0.55},
                    {lat: 37.765, lng: 126.775, risk: 0.5}, {lat: 37.772, lng: 126.788, risk: 0.45},
                    
                    // 기타 경기북부 지역들
                    {lat: 37.593, lng: 127.130, risk: 0.6}, {lat: 37.636, lng: 127.224, risk: 0.65},
                    {lat: 37.826, lng: 127.045, risk: 0.5}, {lat: 37.903, lng: 127.060, risk: 0.55},
                    {lat: 38.146, lng: 127.200, risk: 0.4}, {lat: 37.831, lng: 127.511, risk: 0.35},
                    {lat: 38.096, lng: 127.075, risk: 0.3}, {lat: 37.700, lng: 126.900, risk: 0.45}
                ];
                
                this.currentHeatmapData = this.generateHeatmapPoints(this.heatmapIntensity); 
                this.addRiskOverlayAndMarkers();
                this.addRiskLegend(); // 위험도 범례 추가
    ...
    addRiskLegend() {
        ...
        legendDiv.className = 'risk-legend';
        legendDiv.innerHTML = `
            <div class="font-semibold mb-2 text-gray-800">위험도</div>
            <div class="legend-item">
                <div class="legend-color" style="background-color: #7c2d12;"></div>
                <span class="text-xs">매우높음</span>
            </div>
            <div class="legend-item">
                <div class="legend-color" style="background-color: #dc2626;"></div>
                <span class="text-xs">높음</span>
            </div>
            <div class="legend-item">
            ...
            <div class="legend-item">
                ...
                <span class="text-xs">주의</span>
            </div>
            <div class="legend-item">
                <div class="legend-color" style="background-color: #16a34a;"></div>
                <span class="text-xs">낮음</span>
            </div>
        `;

        // 네이버맵 컨트롤로 범례 추가
        const legendControl = new naver.maps.CustomControl(legendDiv, {
    ...
    generateHeatmapPoints(globalIntensityFactor, timeOfDayFactor = 1.0) {
        return this.baseHeatmapData.map(p => {
            let intensity = p.risk * globalIntensityFactor * timeOfDayFactor;
            intensity += (Math.random() * 0.1 - 0.05); 
            return {
                lat: p.lat, 
                lng: p.lng, 
                risk: Math.max(0, Math.min(1, intensity))
            }; 
        });
    }

    // 네이버맵용 히트맵 오버레이 및 마커 추가 함수
    addRiskOverlayAndMarkers() { 
        ...
        // 히트맵 원으로 위험도 표시
        this.currentHeatmapData.forEach(point => {
            const circle = new naver.maps.Circle({
                map: this.riskMap,
                center: new naver.maps.LatLng(point.lat, point.lng),
                radius: this.heatmapRadius * 50, // 미터 단위로 변환
                fillColor: this.getRiskColor(point.risk),
                fillOpacity: this.heatmapIntensity * point.risk * 0.4,
                strokeColor: this.getRiskColor(point.risk),
                strokeOpacity: 0.6,
                strokeWeight: 1
            });
        ...
        riskAreas.forEach(area => { 
            const marker = new naver.maps.Marker({
                ...
                map: this.riskMap,
                icon: {
                    content: `<div class="w-3 h-3 rounded-full border-2 border-white shadow-md" style="background-color: ${this.getRiskColor(area.risk)};"></div>`,
            ...
            const infoWindow = new naver.maps.InfoWindow({
                content: `
                    <div class="p-3 text-sm" style="min-width:180px;">
            ...
            naver.maps.Event.addListener(marker, 'click', () => {
                this.playSound('map_marker_click');
                infoWindow.open(this.riskMap, marker);
            });
    ...
    startHeatmapAutoUpdate() {
        ...
        this.heatmapAutoUpdateTimer = setInterval(() => {
            ...
            const autoUpdatedData = this.currentHeatmapData.map(p => {
                let newIntensity = p.risk + (Math.random() * 0.04 - 0.02); 
                return {lat: p.lat, lng: p.lng, risk: Math.max(0, Math.min(1, newIntensity))};
            });
        ...
        this.patrolUnitsData = patrolPoints.map(point => {
            if (this.patrolMap) {
                const marker = new naver.maps.Marker({
                    ...
                    map: this.patrolMap,
                    icon: {
                        content: `<div class="p-1 bg-police-blue text-white rounded-full shadow-md flex items-center justify-center w-8 h-8 border-2 border-white">
                                    <i class="fas ${point.icon} text-sm"></i>
                                  </div>`,
                ...
                this.patrolMarkers.push(marker);
                return { ...point, marker, lastActivity: '정보 없음' };
            }
            return { ...point, marker: null, lastActivity: '정보 없음' };
        });
        this.updatePatrolUnitsUI(false);
    }

    getRiskColor(riskValue) { 
        if (riskValue >= 0.8) return '#7c2d12'; 
        if (riskValue >= 0.6) return '#dc2626'; 
        if (riskValue >= 0.4) return '#f97316'; 
        if (riskValue >= 0.2) return '#eab308'; 
        return '#16a34a'; 
    }
    ...
    updateHighRiskAreasUI() { 
        ...
        const sortedRisks = [...this.currentHeatmapData].sort((a, b) => b.risk - a.risk); 
        ...
        topRisks.forEach((riskPoint, index) => { 
            const riskPercent = (riskPoint.risk * 100).toFixed(0); 
            let bgColor = 'bg-yellow-100'; let textColor = 'text-yellow-800'; 
            if (riskPoint.risk >= 0.8) { bgColor = 'bg-red-100'; textColor = 'text-red-800'; } 
            else if (riskPoint.risk >= 0.5) { bgColor = 'bg-orange-100'; textColor = 'text-orange-800'; } 
